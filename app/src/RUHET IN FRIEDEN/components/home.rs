...
use crate::{repository::user::UserRepository, util::error_or::ErrorOr, controllers::common::AuthUser};

use super::base::base_view;

pub fn service<R: UserRepository>(cfg: &mut actix_web::web::ServiceConfig) {
    cfg.service(web::scope("/home").route("", web::get().to(home::<R>)));
}

/// Pass in a vec containing title and href in this exact order to get dropdown with those items.
...
pub async fn home<R: UserRepository>(user: AuthUser, repo: web::Data<R>) -> ErrorOr<Markup> {
    let user = repo.get_session_user(&user.id).await.0?;
