...
use std::{collections::HashMap, sync::Arc, rc::Rc, path::Path};
...
fn Base(cx: Scope) -> Element {
    ...
    let api_client = Rc::new(ApiClient {
        client: get_reqwest_client(cookie_store.clone()),
        cookie_store: cookie_store.clone(),
        base_url: "https://localhost:8443/api/v1"
    });

    use_context_provider(cx, || api_client);
